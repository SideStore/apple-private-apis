...
impl dyn ADIProxy {
    ...
    pub fn provision_device(&mut self) -> Result<()> {
        ...
        let ptm =
            base64_engine.decode(response.get("ptm").unwrap().as_string().unwrap().to_owned())?;
        let tk =
            base64_engine.decode(response.get("tk").unwrap().as_string().unwrap().to_owned())?;
